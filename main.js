...
function doGet(e) {
  ...
  var baseKey = "mrh_" + Utilities.base64EncodeWebSafe(Utilities.newBlob(rawKeyStr).getBytes()).substring(0, 150);
  ...
  function getFileContent(fileName) {
    ...
    if (savedFileId) {
      try { return DriveApp.getFileById(savedFileId).getBlob().getDataAsString(); } 
      catch(err) { props.deleteProperty(fileIdKey); }
    }
    ...
    if (cacheFolderId) {
      try { cacheFolder = DriveApp.getFolderById(cacheFolderId); } 
      catch(err) { cacheFolder = null; props.deleteProperty("PERM_FOLDER_ID"); }
    }
    
    if (!cacheFolder) {
      var rootFolder = DriveApp.getFolderById(ROOT_FOLDER_ID);
      var cacheFolders = rootFolder.searchFolders("title='MRH_Caches' and trashed=false");
    ...
    var files = cacheFolder.searchFiles("title='" + fileName + "' and trashed=false");
    if (files.hasNext()) {
      var file = files.next();
  ...
  if (requestedSubject) {
    ...
    if (passDataStr) {
      try {
        ...
        return ContentService.createTextOutput(JSON.stringify({ error: "Internal Security Error" })).setMimeType(ContentService.MimeType.JSON);
      }
    }

    var safeName = Utilities.base64EncodeWebSafe(Utilities.newBlob(requestedSubject).getBytes());
  ...
  try {
    ...
    if (responseString.length > 0) {
      ...
      for (var c = 0; c < chunksTotal; c++) {
        cacheObj[baseKey + "_" + c] = responseString.substring(c * maxStr, (c + 1) * maxStr);
      }
      cache.putAll(cacheObj, 21600); 
    }
  } catch (err) {}
  
  return ContentService.createTextOutput(responseString).setMimeType(ContentService.MimeType.JSON);
}
  ...
  try {
    ...
    if (requestType === "telemetry") {
      ...
      if (lock.tryLock(3000)) {
        ...
        queue.push([new Date().toISOString(), data.userId, data.action, data.details]);
        cache.put("TELEMETRY_QUEUE", JSON.stringify(queue), 21600);
        lock.releaseLock();
      }
      return ContentService.createTextOutput(JSON.stringify({"status": "success", "route": "telemetry"})).setMimeType(ContentService.MimeType.JSON);
    }

    if (requestType === "admin_get_subjects") {
      if (data.token !== ADMIN_TOKEN) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Unauthorized access."})).setMimeType(ContentService.MimeType.JSON);

      var rootFolder = DriveApp.getFolderById(ROOT_FOLDER_ID);
      var cacheFolders = rootFolder.searchFolders("title='MRH_Caches' and trashed=false");
      if (!cacheFolders.hasNext()) return ContentService.createTextOutput(JSON.stringify([])).setMimeType(ContentService.MimeType.JSON);
      
      var cacheFolder = cacheFolders.next();
      var summaryFiles = cacheFolder.searchFiles("title='MRH_Summary.json' and trashed=false");
      var passFiles = cacheFolder.searchFiles("title='MRH_Passwords.json' and trashed=false");
      
      var summaryData = [];
      var passMap = {};

      if (summaryFiles.hasNext()) { try { summaryData = JSON.parse(summaryFiles.next().getBlob().getDataAsString()); } catch(err) {} }
      if (passFiles.hasNext()) {
        try {
          var passData = JSON.parse(passFiles.next().getBlob().getDataAsString());
          for (var i = 0; i < passData.length; i++) { passMap[passData[i].Subject] = passData[i].password; }
        } catch(err) {}
      }

      var combined = summaryData.map(function(item) {
        var p = passMap[item.Subject] || "";
        return { Subject: item.Subject, QuestionCount: item.QuestionCount, Locked: item.Locked, Password: p, password: p, IsFolder: item.IsFolder || false };
      });
    ...
    if (requestType === "verify_admin") {
      return ContentService.createTextOutput(JSON.stringify({"status": data.token === ADMIN_TOKEN ? "success" : "error"})).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (requestType === "verify_user") {
      ...
      if (cachedUsers) {
        ...
        if (!userSheet) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "USERS sheet not found in database."})).setMimeType(ContentService.MimeType.JSON);
      ...
      for (var u = 1; u < userData.length; u++) {
        if (userData[u][0] === data.username && userData[u][1] === data.password) {
    ...
    if (requestType === "admin_update_password") {
      ...
      var passSheet = passSs.getSheetByName("DECK") || passSs.insertSheet("DECK"); 
    ...
    if (requestType === "submit_report") {
      ...
      var sheet = ss.getSheetByName("MRH_Reports") || ss.insertSheet("MRH_Reports");
      sheet.appendRow([
        Utilities.getUuid(), new Date().toISOString(), data.questionId, data.subject, data.questionText, data.errorType, data.comments, "Pending", "",
        data.optionA || "", data.optionB || "", data.optionC || "", data.optionD || "", data.correctAnswer || ""
      ]);
      return ContentService.createTextOutput(JSON.stringify({"status": "success"})).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (requestType === "get_reports") {
      ...
      for (var i = 1; i < rows.length; i++) {
        ...
        reports.push({
          id: rows[i][0], timestamp: rows[i][1], questionId: rows[i][2], subject: rows[i][3], questionText: rows[i][4], errorType: rows[i][5],
          comments: rows[i][6], status: status, optionA: rows[i][9] || "", optionB: rows[i][10] || "", optionC: rows[i][11] || "", optionD: rows[i][12] || "", correctAnswer: rows[i][13] || ""
        });
      }
      return ContentService.createTextOutput(JSON.stringify(reports.reverse())).setMimeType(ContentService.MimeType.JSON);
    }

    if (requestType === "admin_resolve_report") {
      if (data.token !== ADMIN_TOKEN) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Unauthorized."})).setMimeType(ContentService.MimeType.JSON);
      ...
      return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Report not found."})).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (requestType === "admin_update") {
      ...
      var passMap = {};
      for (var p = 1; p < passData.length; p++) {
        if (passData[p][0]) passMap[passData[p][0]] = String(passData[p][1] || "");
      }

      var fileIterator = rootFolder.searchFiles("mimeType='application/vnd.google-apps.spreadsheet' and trashed=false");
      var localFiles = [];
      ...
      for (var i = 0; i < updates.length; i++) {
        ...
        if (update.newName !== update.oldName) {
            ...
            if (update.password !== undefined && update.password !== "") passMap[update.newName] = update.password;
            else if (oldPass && (update.password === undefined || update.password === oldPass)) passMap[update.newName] = oldPass;
        } else {
        ...
        if (update.newName === update.oldName) { renamedCount++; continue; }

        var oldParts = update.oldName.split("::");
        var newParts = update.newName.split("::");
        
        var possibleName1 = oldParts[oldParts.length - 1].trim();
        var possibleName2 = oldParts.length > 1 ? oldParts[oldParts.length - 2].trim() : possibleName1;
                ...
                if (isSingleSheet) {
                    ...
                    if (fileName === possibleName2) {
                        ...
                        if (sheet) {
                            ...
                            var newFileName2 = newParts.length > 1 ? newParts[newParts.length - 2].trim() : newSheetName;
                            if (fileName !== newFileName2) {
                                file.setName(newFileName2);
                                localFiles[fIdx].name = newFileName2;
                            }
                            if (sheet.getName() !== newSheetName) sheet.setName(newSheetName);
                            foundAndRenamed = true; break;
                        }
                    }
                }
      ...
      if (outData.length > 0) passSheet.getRange(1, 1, outData.length, 2).setValues(outData);
      
      triggerBuildDatabaseCache();
      return ContentService.createTextOutput(JSON.stringify({"status": "success", "message": "Successfully updated " + renamedCount + " items!"})).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (requestType === "admin_edit_question") {
      ...
      if (!subject || !questionId) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Missing subject or question ID."})).setMimeType(ContentService.MimeType.JSON);

      var rootFolder = DriveApp.getFolderById(ROOT_FOLDER_ID);
      var parts = subject.split("::");
      var possibleName1 = parts[parts.length - 1].trim();
      var possibleName2 = parts.length > 1 ? parts[parts.length - 2].trim() : possibleName1;
      
      var query = "mimeType='application/vnd.google-apps.spreadsheet' and trashed=false and (title='" + possibleName1.replace(/'/g, "\\'") + "' or title='" + possibleName2.replace(/'/g, "\\'") + "')";
      ...
      while (files.hasNext()) {
        ...
        if (sheets.length === 1 && file.getName() === possibleName1) { targetSheet = sheets[0]; break; }
        else {
          if (file.getName() === possibleName2) { targetSheet = ss.getSheetByName(possibleName1); if (targetSheet) break; }
          else if (file.getName() === possibleName1) { targetSheet = ss.getSheetByName(possibleName1); if (targetSheet) break; }
        }
      }

      if (!targetSheet) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Target sheet not found."})).setMimeType(ContentService.MimeType.JSON);

      var idIndex = parseInt(questionId.split("-").pop(), 10);
      var rowNumber = idIndex + 1; 

      if (isNaN(rowNumber) || rowNumber <= 1 || rowNumber > targetSheet.getLastRow()) return ContentService.createTextOutput(JSON.stringify({"status": "error", "message": "Invalid question row."})).setMimeType(ContentService.MimeType.JSON);

      targetSheet.getRange(rowNumber, 1, 1, 5).setValues([[
      ...
      else if (ans === "B") targetSheet.getRange(rowNumber, 3).setFontWeight("bold");
      else if (ans === "C") targetSheet.getRange(rowNumber, 4).setFontWeight("bold");
      else if (ans === "D") targetSheet.getRange(rowNumber, 5).setFontWeight("bold");
      ...
      try {
        ...
        if (chunkCountStr) {
           ...
           for (var c = 0; c < chunkCount; c++) keysToRemove.push(baseKey + "_" + c);
           cache.removeAll(keysToRemove);
        }
      } catch (e) {}

      return ContentService.createTextOutput(JSON.stringify({"status": "success", "message": "Question updated successfully!"})).setMimeType(ContentService.MimeType.JSON);
    }
...
function buildDatabaseCache() {
  ...
  try {
    ...
    if (stateJson) {
      ...
      var rootFolder = DriveApp.getFolderById(ROOT_FOLDER_ID);
      var cacheFolders = rootFolder.searchFolders("title='MRH_Caches' and trashed=false");
      var cacheFolder = cacheFolders.hasNext() ? cacheFolders.next() : rootFolder.createFolder("MRH_Caches");
      ...
      if (!isFullRebuild) {
        ...
        if (summaryFiles.hasNext()) {
          try {
            var summaryData = JSON.parse(summaryFiles.next().getBlob().getDataAsString());
            for (var i = 0; i < summaryData.length; i++) oldSummaryMap[summaryData[i].Subject] = summaryData[i].QuestionCount;
          } catch(e) { isFullRebuild = true; }
        } else { isFullRebuild = true; }
      }

      var fileTimestamps = {};
      try { fileTimestamps = JSON.parse(scriptProps.getProperty('MRH_FILE_TIMESTAMPS') || '{}'); } catch(e) {}
    ...
    while (state.folderQueue.length > 0) {
      ...
      if (Date.now() - startTime > MAX_RUNTIME_MS) {
        ...
        ScriptApp.newTrigger("resumeBuildDatabaseCache").timeBased().after(10000).create();
        Logger.log("Safe runtime limit hit (4.5 min). Scheduled continuation trigger.");
        return;
      }
      ...
      while (files.hasNext()) {
        ...
        if (fileName === "MRH_Telemetry" || fileName === "MRH_Passwords" || fileName.indexOf("MRH_Reports") > -1) continue;
        ...
        if (!isModified && !state.isFullRebuild) {
          ...
          for (var oldSubj in state.oldSummaryMap) {
            if (oldSubj === fileBase || oldSubj.indexOf(fileBase + "::") === 0) {
        ...
        for (var i = 0; i < sheets.length; i++) {
          ...
          subjectName += isSingleSheetFile ? fileName : (fileName === sheet.getName() ? fileName : (fileName + "::" + sheet.getName()));

          var qData = extractQuestionsOptimized(sheet, subjectName);

          if (qData.length > 0) {
            state.activeSubjects[subjectName] = true;
            var safeName = Utilities.base64EncodeWebSafe(Utilities.newBlob(subjectName).getBytes());
            updateOrCreateCacheFile(cacheFolder, "SUBJ_" + safeName + ".json", JSON.stringify(qData));
      ...
      while (subFolders.hasNext()) {
        ...
        var nextPath = pathPrefix ? (pathPrefix + "::" + subFolder.getName()) : subFolder.getName();
        state.folderQueue.push({ id: subFolder.getId(), pathPrefix: nextPath });
      }
    }

    // FINALIZATION PHASE
    scriptProps.setProperty('MRH_FILE_TIMESTAMPS', JSON.stringify(state.newFileTimestamps));
    ...
    for (var key in state.summaryMap) {
      ...
      summaryArray.push({ Subject: key, QuestionCount: state.summaryMap[key], Locked: storedPass !== "" });
      passwordsArray.push({ Subject: key, password: storedPass });
    }

    for (var passKey in passMap) {
      if (!processedKeys[passKey] && passMap[passKey] !== "") {
         summaryArray.push({ Subject: passKey, QuestionCount: 0, Locked: true, IsFolder: true });
         passwordsArray.push({ Subject: passKey, password: passMap[passKey] });
      }
    }

    updateOrCreateCacheFile(cacheFolder, "MRH_Summary.json", JSON.stringify(summaryArray));
    updateOrCreateCacheFile(cacheFolder, "MRH_Passwords.json", JSON.stringify(passwordsArray));

    var cachedFiles = cacheFolder.searchFiles("title contains 'SUBJ_' and trashed=false");
    while (cachedFiles.hasNext()) {
      ...
      if (cName.indexOf("SUBJ_") === 0 && cName.indexOf(".json") > 0) {
        ...
        try {
          var decodedSubject = Utilities.newBlob(Utilities.base64DecodeWebSafe(base64Part)).getDataAsString();
          if (!state.activeSubjects[decodedSubject]) hardDeleteFile(cFile);
        } catch (e) { hardDeleteFile(cFile); }
      }
    }
...
function getOrCreateTelemetrySheet() {
  var rootFolder = DriveApp.getFolderById(ROOT_FOLDER_ID);
  var files = rootFolder.searchFiles("title='MRH_Telemetry' and mimeType='application/vnd.google-apps.spreadsheet'");
  ...
  if (lock.tryLock(10000)) {
    ...
    if (currentCache) {
      ...
      if (queue.length > 0) {
        ...
        sheet.getRange(sheet.getLastRow() + 1, 1, queue.length, 4).setValues(queue);
...
function permanentlyDeleteOldCaches() {
  var trashedFolders = DriveApp.searchFolders("title='MRH_Caches' and trashed=true");
  var count = 0;
  ...
  while (trashedFolders.hasNext()) {
    ...
    try {
      ...
      Logger.log("Failed to delete folder ID: " + folder.getId() + " - " + e.message);
    }
  }
  Logger.log("Success! Permanently deleted " + count + " old cache folders from the trash.");
}

function hardDeleteFile(file) {
  try { Drive.Files.remove(file.getId()); } 
  catch (e) { file.setTrashed(true); }
}